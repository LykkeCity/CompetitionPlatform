$(function () {
    ...
    $('.add-row').click(function () {
        ...
        var projName = $("#projectName option:selected").text();
        var projId = $("#projectName option:selected").val();

        var markup = "<tr><td><input type='checkbox' name='record'></td><td>" + place + "</td><td>" + projName + '</td><td style="display: none;">' + projId +'</td></tr>';
        $('table tbody').append(markup);

        $('#place').val('');
        $('#projectName').val('');

        var table = $('#streamProjectsTable').tableToJSON(
            {
                ignoreColumns: [0], 
                headings: ['Place', 'ProjectName', 'ProjectId']
            });

        $('#SerializedStream').val(JSON.stringify(table));
    });

    // Find and remove selected table rows
    $('.delete-row').click(function () {
        $('.streamProjectsTable tbody').find('input[name="record"]').each(function () {
            if ($(this).is(':checked')) {
    ...
    $('#StreamType').change(function () {
        var val = $('#StreamType option:selected').text();
        if (val === 'Existing') {
            $('#hiddenStreamElements').show();
            $('.existingStreamDropdown').show();
            $('.newStreamNameTextbox').hide();
        }
        if (val === 'New') {
            $('#streamProjectsTable tr').remove();

            $('#hiddenStreamElements').show();
            $('.existingStreamDropdown').hide();
            $('.newStreamNameTextbox').show();
        }
        if (val === 'None') {
            $('#hiddenStreamElements').hide();
        }
    });

    $('#ExistingStreamId').change(function() {
        var val = $('#ExistingStreamId option:selected').val();
        $('#streamProjectsTableEdit').load('/Project/GetEditStreamsTable?streamId=' + val);
    });
});
